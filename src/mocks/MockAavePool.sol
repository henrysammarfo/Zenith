 ...
 */
contract MockAavePool is IAaveLendingPool, ERC20 {
    ...
    constructor(address _asset) ERC20("Mock Aave Token", "mAAVE") {
        ASSET = ERC20(_asset);
    ...
    function withdraw(address _asset, uint256 amount, address to) external override returns (uint256) {
        ...
        require(balanceOf(msg.sender) >= amount, "MockAavePool: Insufficient balance");
    ...
    function setLiquidityRate(uint256 newRate) external {
        ...
        emit ReserveDataUpdated(address(ASSET), newRate, 0, 0, 1e27, 1e27);
    }
    
    // balanceOf is already provided by ERC20
}
